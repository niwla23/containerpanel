...
def index(request):
    if request.user.is_authenticated:
        containers = []
        for i in Server.objects.order_by('description'):
            allowed_users = []
...
def choose_template(request):
    ...
    return render(request, 'api/choose_template.html', {"templates": templates})
...
def add_new(request):
    if request.method == "GET":
        ...
        print(form)
        if form.is_valid():
            with open(f"app_templates/{form.cleaned_data['template']}.yml", "r") as file:
            ...
            sftp_base_directory = form.cleaned_data["sftp_base_directory"]
            ...
            config = {
                ...
                "services": {
                    ...
                    "sftp": {
                        "image": "atmoz/sftp",
                        "restart": "unless-stopped",
                        "command": f"user:{sftp_password}:1000",
                        "ports": [int(sftp_port)],
                        "volumes": f"{sftp_base_directory}:/home/user/upload"
                    }
                }
            }
    ...
    return render(request, 'api/add_new.html', {"form": form})
...
def container(request, container_id):
    db_data = get_object_or_404(Server, pk=container_id)
    if can_manage_container(db_data, request.user.id):
        ...
        return render(request, 'api/container.html', data)
    else:
...
def i_want_logs(sid, message):
    try:
        ...
        db_data = get_object_or_404(Server, pk=message["container_id"])
